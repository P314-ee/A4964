...
//----------------------------------------------
#define oCSL                HAL_GPIO_WritePin(A4964_CS_GPIO_Port, A4964_CS_Pin, GPIO_PIN_RESET)
#define oCSH                HAL_GPIO_WritePin(A4964_CS_GPIO_Port, A4964_CS_Pin, GPIO_PIN_SET)
#define wakeUp              HAL_GPIO_WritePin(A4964_WAKEUP_GPIO_Port, A4964_WAKEUP_Pin, GPIO_PIN_SET)
#define SPI_Open()          
#define SPI_Close()         
#define SPI_ExchangeByte(x) SPI_Exchange_Byte(x)

//==============================================//
//				Global Variable						//
//==============================================//
uint16_t regConfig[32] = {0};
uint16_t regConfig_Read[32] = {0};
...
//				Function				//
//==============================================//
uint8_t configStopOnFail (uint8_t esf);
uint8_t configVLRvoltage (uint8_t vlr);
uint8_t configGateVoltage (uint8_t vrg);
uint8_t configOperatingMode (uint8_t opm);
uint8_t configWakeMode (uint8_t lwk);
uint8_t configPWMSense (uint8_t ipi);
uint8_t configCurrentLimitEnable (uint8_t dil);
uint8_t configMotorMode (uint8_t mod);
uint8_t configLIN (uint8_t len);
uint8_t configSLP (uint8_t slp);
uint8_t configOvermodulation (uint8_t ovm);
uint8_t configDriveMode (uint8_t drm);
uint8_t configBRAKE (uint8_t brk);
uint8_t configDIR (uint8_t dir);
uint8_t configRUN (uint8_t run);
uint8_t configSpeedAccel (uint16_t accHzX10,uint8_t gain);
uint8_t configSpeedFlags (uint8_t dv,uint8_t df);
uint8_t configSpeed (uint8_t res, uint16_t lthr, uint16_t hthr);
uint8_t configSenseMaxThresh (uint8_t mit);
uint8_t configVDSOvervoltage(uint16_t mv);
uint8_t configVDSFaultQualifier(uint8_t mod,uint16_t ns);
uint8_t configPWM(uint8_t mod,uint8_t pmd,uint16_t freq);
uint8_t configDither(uint8_t ditherTime,uint8_t dwellTime,uint8_t stepCnt);
uint8_t configBridge(uint8_t gain, uint16_t dt);
uint8_t configGateCurrent(uint8_t gate, uint8_t cur1, uint8_t cur2);
uint8_t configGateSlew(uint16_t nsOn, uint16_t nsOff);
uint8_t configCurrentLimit (uint16_t ns,uint8_t scale);
uint8_t configStartupAlign(uint16_t ms,uint8_t dutyp);
uint8_t configStartupMotor(uint8_t coast,uint8_t rsc,uint16_t km,uint8_t rampt);
uint8_t configWindmill(uint8_t mod,uint8_t freq,uint8_t dutyp);
uint8_t configRamp(uint16_t startFreqX10,uint8_t startDuty,uint16_t endFreqX10, uint8_t endDuty);
uint8_t configRampStep(uint16_t stepms, uint8_t stepfreq);
uint8_t configBEMFDetectWindow(uint16_t degX10);
uint8_t configBEMF(uint8_t samplecnt, uint8_t filtertime);
uint8_t configCommSteadyPow2(int8_t np,int8_t ni);
uint8_t configCommTransientPow2(int8_t np,int8_t ni);
uint8_t configPhaseAdvance(uint8_t mod, uint8_t gain,uint16_t degX10);
uint8_t configDIAG (uint8_t DGS);
uint8_t configDataOutput (uint8_t RBS);

uint8_t A4964_SPI (uint16_t Data ,uint8_t WR ,uint16_t *rcvData);
...
// A4964_SPI
//==============================================
uint8_t A4964_Demand_Input (uint16_t DemandInput)
{
    uint8_t stat;
    uint16_t wData;
...
// A4964_SPI
//==============================================
uint32_t A4964_ReadBack_Select (uint8_t rSel)
{
    uint8_t stat;
    uint16_t wData;
    uint32_t tmp;
...
uint8_t A4964_ReadReg(uint8_t regAddr, uint16_t *regValue)
{
    uint16_t cmd = 0;
    uint16_t dummyRead;
...
void A4964_ReadAllRegs(void)
{
    ...
    printf("===== A4964 Register Dump =====\r\n");

    for (uint8_t i = 0; i < 30; i++)
    {
        if (A4964_ReadReg(i, &regValue))
        {
            regConfig_Read[i] = regValue;
        }
        else
        {
            regConfig_Read[i] = 0xFFFF;
        }

        HAL_Delay(10); // tránh đọc quá nhanh
    }
}
...
void A4964_init2 (void)
{
    ...
    configSenseMaxThresh(MIT_200MV); // REG 7

    // -- Current Limit Scale
    // n = 0~15
    // scale (0~100)%
    configCurrentLimit(1800, 100); // REG 6 Current Limit Blank Time = 1800 ns, Current Limit Scale = 100%
    ...
    // DF_10
    //------------------------------------------
    configSpeedFlags(DV_DISABLED, DF_1); // REG 22
    ...
    // fSL = 0 -> n = 0
    //----------------------------------------------
    // -- Overspeed (High-Speed) Threshold
    // fSH = [127 + (n * 128)] * fRES (Hz)
    // fSH = 820 -> n = 15

    configSpeed(SR_0_2HZ, 0, 60); // REG 22 & 23
    ...
    configSpeedAccel(500, 31); // REG 21


    //----------------------------------------------
    // uint8_t configStopOnFail (uint8_t sof)
    //----------------------------------------------
    // -- Enable Stop On Fail Select
    // ESF_NOSTOP
    // ESF_STOP
    configStopOnFail(ESF_STOP); // REG 25

    //----------------------------------------------
    // uint8_t configVLRvoltage (uint8_t sof)
    //----------------------------------------------
    // -- Logic Regulator Voltage
    // VLR_3_3V
    // VLR_5V
    configVLRvoltage(VLR_3_3V); // REG 25
    ...
    // VRG_8V
    // VRG_11V
    configGateVoltage (VRG_11V); // REG 25
    ...
    // OPM_SPI_ONLY
    // OPM_STAND_ALONE
    configOperatingMode(OPM_SPI_ONLY); // REG 25
    ...
    // IPI_ACT_HIGH
    // IPI_ACT_LOW
    configPWMSense(IPI_ACT_HIGH); // REG 25
    ...
    // DIL_ENABLED
    // DIL_DISABLED
    configCurrentLimitEnable(DIL_ENABLED); // REG 25
    ...
    // CM_OPEN_LOOP
    configMotorMode(CM_CLOSE_LOOP_SPEED);  // REG 25

    //----------------------------------------------
    // uint8_t configDriveMode(uint8_t mod)
    //------------------f----------------------------
    // -- Drive Mode Select
    // DRM_SINUSOIDAL
    // DRM_TRAPEZOIDAL
    configDriveMode(DRM_TRAPEZOIDAL); // REG 27
    ...
    // OVM_125P
    // OVM_150P
    configOvermodulation(OVM_100P); // REG 27

// PWM
    //----------------------------------------------
    // uint8_t configPWM(uint8_t phase,uint8_t align,uint16_t freq)
    //----------------------------------------------
    // -- ModulationÈM Mode Select
    // MOD_PHASE3
    // MOD_PHASE2
    //------------------------------------------
    ...
    configPWM(MOD_PHASE3, PMD_ALIGN_CENTER, 20000); // REG 0, Note 50000HZ = 50khz
    ...
    //==============================================
    configDither(DP_1600NS, DD_1MS, 0); // 0 steps = dither disabled  // REG 1
    ...
    configBridge(SA_10, 600); // gain, dead time ns  // REG 2

// Gate
    //----------------------------------------------
    // uint8_t configGateCurrent(uint8_t sel, uint8_t cur1, uint8_t cur2)
    //----------------------------------------------
...
// Current Limiting
    ...
    // return spi write success of failed
    //==============================================
    configCurrentLimit(1000, 100); // blank time ns, scale % // REG 6


// VDS
    //==============================================
    // uint8_t configVDSOvervoltage(uint16_t mvX100) -- Overvoltage Threshold
    //----------------------------------------------
    // -- VDS Overvoltage Threshold
    // n = 0~63
    // VDST = 0 ~ 3150mV
    //----------------------------------------------
    configVDSOvervoltage(3150); // mv  // REG 7


    //----------------------------------------------
    // uint8_t configVDSFaultQualifier(uint8_t mod,uint16_t ns) -- VDS Fault Qualifier
    //----------------------------------------------
    ...
    configVDSFaultQualifier(VDQ_DEBOUNCE, 3150); // ns  // REG 8


// Startup
    //----------------------------------------------
    // uint8_t cconfigStartupAlign (hold time ms, duty)
    //----------------------------------------------
    // -- Alignment (Hold) Time
    ...
    configStartupAlign(2000, 80); // hold time ms, duty cycle percent  // REG 15


    //----------------------------------------------
    // uint8_t configStartupMotor(STM_COAST_OFF, RSC_RESTART_OFF, 75, HR_0P)
    //----------------------------------------------
    ...
    configStartupMotor(STM_COAST_OFF, RSC_RESTART_OFF, 75, HR_100P);  // REG 16
    ...
    configWindmill(WIN_DISABLED, 7, 50); // freq hz, duty cycle percent  // REG 17
...
// Ramping
    ...
    // input range: 0 ~ 100
    // REG 18 // REG 19
    configRamp(10, 30, 15, 80);  // start(0.5~8.0) hz, duty(0~100), end freq (10.0~47.5), duty(0~100)


    //------------------------------------------------------
    // uint8_t configRampStep(uint16_t ms, uint8_t freq); // step time ms, step freq
    //----------------------------------------------
    ...
    // input range : 0 ~ 15
    // REG 20
    configRampStep(300, SFS_1HZ); // step time (10 ms to 300 ms.) ms, step freq
    ...
    //------------------------------------------------------
    // -- PHASE ADVANCE
    // n = 0 ~ 63 , The range of �cADV is 0 to 43.4o and 60o
    // input range : 0 ~ 600
    //------------------------------------------------------
    configPhaseAdvance(PAM_AUTO, KIP_GAIN_1, 0); // REG 26
...
// BEMF
    ...
    //------------------------------------------------------
    // The range of �cBW is 1.4o to 43.4o and 60o.
    // input range: 0 ~ 600
 // REG 13
    configBEMFDetectWindow(100);    // degrees(5.6)*10


    //------------------------------------------------------
    // configBEMF(uint8_t BS, uint8_t BF)
    //------------------------------------------------------
 ...
 // REG 14
    ...
    // 2^-1 = 1/2   | 2^5  = 32 |
    // REG 11
    configCommSteadyPow2(2^1, 2^2);     // P=2^0=1, I=2^0=1
     // REG 12
    configCommTransientPow2(2^1, 2^2);  // P=2^0=1, I=2^0=1
...
uint8_t configStopOnFail (uint8_t esf)
{
    ...
    wData = regConfig[r25_SYSTEM_CONFIG];
    //------------------------------------------
    // ESF(b9) Enable Stop On Fail Select
    if (esf)
        wData |= (1<<9);
    ...
    if (stat)
        regConfig[r25_SYSTEM_CONFIG] = wData;
...
// tPW = 20.10 us + (n * 0.8 us) when PMD = 0
// tPW = 20.05 us + (n * 0.8 us) when PMD = 1
// n = 0~63 , This is equivalent to 50 kHz to 14.2 kHz
...
//==============================================
// uint8_t cconfigStartupAlign(100, 3)
//----------------------------------------------
// Register 15: Startup Configuration
//----------------------------------------------
...
//==============================================
// uint8_t configStartupMotor(COAST_OFF, DESYNC_RESTART_OFF, 750, RAMP_0P);
//----------------------------------------------
// Register 16: Startup Configuration
//----------------------------------------------
...
//==============================================
// uint8_t configRamp(uint16_t startFreqX10,uint8_t startDuty,uint16_t endFreqX10, uint8_t endDuty); // start freqx10 hz, duty, end freqx10, duty
//----------------------------------------------
...
{
    uint8_t stat;
    uint16_t nSF2,nSF1,nSD2,nSD1,wData,tmp;
...
// �cBW = (n + 1) * 1.4o(elec) |n=0..30
// �cBW = 60o(elec)            |n=31
// n = 0~31 , The range of �cBW is 1.4o to 43.4o and 60o.
// input range: 0 ~ 600
//----------------------------------------------
// return :
// 1: write success
...
//==============================================
// uint8_t configBEMF(uint8_t samplecnt, uint8_t filtertime); // mode, gain, degrees*10
//----------------------------------------------
// Register 14: BEMF Sampling
//----------------------------------------------
...
//==============================================
// uint8_t configPhaseAdvance(uint8_t mod, uint8_t gain,uint16_t degX10); // mode, gain, degrees*10
//----------------------------------------------
